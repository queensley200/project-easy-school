...
class FinanceSummaryAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        monthly_salaries = (
            TeacherSalary.objects.annotate(month=Trunc("paid_on", "month"))
            .values("month")
            .annotate(total=Sum("total_amount"))
        )
        monthly_fees = (
            StudentFee.objects.annotate(month=Trunc("date_submitted", "month"))
            .values("month")
            .annotate(total=Sum("total_amount"))
        )
        ...
        graph_data = {
            "labels": labels,
            "datasets": [
                {"label": "Teacher", "backgroundColor": "#F5DD5D", "data": salaries},
                {"label": "Student", "backgroundColor": "#44B78B", "data": fees},
            ],
        }
